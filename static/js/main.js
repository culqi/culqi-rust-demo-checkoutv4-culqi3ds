    ...
    if (event.origin === window.location.origin) {
      ...
      if (parameters3DS) {
        ...
        if (paymenType === "cargo") {
          ...
          objResponse = JSON.parse(responseCharge.data).object;
          ...
          objResponse = JSON.parse(responseCard.data).object;
...
window.culqi = async () => {
  ...
  if (Culqi.token) {
    ...
    if (paymenType === "cargo") {
      ...
      objResponse = JSON.parse(responseCharge.data);     
      console.log(objResponse.action_code);
      statusCode = responseCharge.statusCode;
    } else {
      ...
      objResponse = JSON.parse(responseCard.data);
      statusCode = responseCard.statusCode;
    }
